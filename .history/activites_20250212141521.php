...
<?php
if (!empty($_POST['activites'])) {  // Vérifie si au moins une activité est cochée
	foreach ($uniksubactivite as $subsport) {
		echo '<input type="checkbox" name="sousactivites[]" value="' . htmlspecialchars($subsport) . '" id="sub_' . htmlspecialchars($subsport) . '">';
		echo '<label for="sub_' . htmlspecialchars($subsport) . '">' . htmlspecialchars($subsport) . '</label><br>';
	}
} else {
